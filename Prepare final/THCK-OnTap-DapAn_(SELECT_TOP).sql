...
BEGIN
	...
	DECLARE @i int = 1, @MADT char(10), @GVCNDT char(10), @SL_GVTG int;
	WHILE ( @i <= @SL_DETAI )
	BEGIN
		SELECT '== DE TAI #  ' + CAST(@i as char(3)) + ' / ' + CAST(@SL_DETAI as char(3)) + '===============================';
		SELECT TOP(@i) @MADT = MADT, @GVCNDT = GVCNDT
			FROM DETAI
			WHERE (@Tu_Ngay <= NGAYBD AND NGAYBD <= @Den_Ngay )
				OR (@Tu_Ngay <= NGAYKT AND NGAYKT <= @Den_Ngay )
				OR (NGAYBD <= @Tu_Ngay AND @Den_Ngay <= NGAYKT )
...
EXEC spHienThi_DSGV_ThamGia_DeTai '2000-1-1', '2010-1-1'
